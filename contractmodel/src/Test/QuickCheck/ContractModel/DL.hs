...
data DLTest state =
    BadPrecondition [TestStep state] [FailedStep state] state
        -- ^ An explicit `action` failed its precondition (@[Action](#v:Action)@), or an assertion
        --   failed (`Assert`).
...
toDLTest (BadPrecondition steps acts s) =
  DL.BadPrecondition (toDLTestSteps steps) (map conv acts) (dummyModelState s)
    where
        conv (Action (ActWaitUntil _ n)) = StateModel.Some (WaitUntil n)
        conv (Action a)                  = StateModel.Some (ContractAction (isBind a) (actionOf a))
        conv (Assert e)                  = StateModel.Error e
...
fromDLTest (DL.BadPrecondition steps acts s) =
  ...
  where conv :: StateModel.Any (StateModel.Action (ModelState s)) -> [FailedStep s]
        conv (StateModel.Some (ContractAction _ act)) = [Action $ NoBind (StateModel.Var 0) act]
        conv (StateModel.Some (WaitUntil n))          = [Action $ ActWaitUntil (StateModel.Var 0) n]
        conv (StateModel.Error e)                     = [Assert e]
...
fromDLTestStep (DL.Do (v StateModel.:= ContractAction b act)) = [Do $ if b then Bind v act else NoBind v act]
fromDLTestStep (DL.Do (v StateModel.:= WaitUntil n))          = [Do $ ActWaitUntil v n]
...
forAllDL :: (ContractModel state, DynLogicModel state, Testable p) => DL state () -> (Actions state -> p) -> Property
forAllDL dl prop = DL.forAllMappedDL toDLTest fromDLTest fromStateModelActions dl prop

forAllDL_ :: (ContractModel state, DynLogicModel state, Testable p) => DL state () -> (Actions state -> p) -> Property
forAllDL_ dl prop = DL.forAllMappedDL_ toDLTest fromDLTest fromStateModelActions dl prop

forAllUniqueDL :: (ContractModel state, DynLogicModel state, Testable p) => Int -> ModelState state -> DL state () -> (Actions state -> p) -> Property
...
class ContractModel s => DynLogicModel s where
  restricted :: Action s -> Bool
  restricted _ = False

instance (ContractModel s, DynLogicModel s) => DL.DynLogicModel (ModelState s) where
